...
class App:
  def __init__(self, master):
    ...
    self.ey.grid(row=1,column=1)
    self.ey.insert(0,"0")
    self.ez = Entry(mainframe)
    self.ez.grid(row=2,column=1)
    self.ez.insert(0,"1")
    ...
    self.pos_pub = rospy.Publisher('ref_global', TransformStamped)
    self.send_position()
...
rospy.init_node('ref_generation', anonymous=True)

root = Tk()
root.title("Reference point generation")
app = App(root)
